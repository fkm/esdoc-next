...
In order to achieve these two goals, ESDoc produces a practical document, measures the coverage, integrates the test code and more.

## Difference between ESDoc and JSDoc
JSDoc is the most popular JavaScript API Documentation tool.
...
ESDoc displays the import path of a class/function into the document.<br/>
However, the import path may be different from the real import path because usually ES modules are transpiled to use it.

So, [ESDoc Import Path Plugin](https://github.com/esdoc/esdoc-plugins/tree/master/esdoc-importpath-plugin) converts the import path to the real import path.
